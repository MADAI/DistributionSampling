...
::GenerateMaxiMin(
    ...
    int number_of_tries)
{
  assert(number_of_tries > 1);
  std::vector< Sample > bestLHC;
  ...
  for (int i = 0; i < number_of_tries; ++i) {
    std::vector< Sample > lhc =
      this->Generate(numberOfTrainingPoints, parameters);
    double minDist2 = std::numeric_limits< double >::infinity();
    for (size_t j = 0; j < lhc.size(); ++j) {
      const std::vector< double > & u = lhc[j].m_ParameterValues;
      for (size_t k = 0; k < j; ++k) {
        const std::vector< double > & v = lhc[k].m_ParameterValues;
        double dist2 = 0.0;
  ...
  assert(numberOfTrainingPoints == static_cast< int >(bestLHC.size()));
  return bestLHC;
}
